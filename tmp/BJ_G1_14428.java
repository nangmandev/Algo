/**
 ...
 @git https://github.com/cbnu2017038040/SSAFY_AL/blob/main/algo/src/algo/BJ_G1_14428.java
 @youtube
 @performance 70828KB, 944ms
 @category 세그먼트 트리
 @note
...
public class BJ_G1_14428 {
	...
	private static int getSum(int left, int right) {
		...
		while(left <= right) {
			if(left % 2 == 1) {
				if(valueArr[rtn] > valueArr[arr[left]]) {
					rtn = arr[left];
				}
				else if(valueArr[rtn] == valueArr[arr[left]]) {
					rtn = Math.min(rtn, arr[left]);
				}
				left++;
			}
			if(right % 2 == 0) {
				if(valueArr[rtn] > valueArr[arr[right]]) {
					rtn = arr[right];
				}
				else if(valueArr[rtn] == valueArr[arr[right]]) {
					rtn = Math.min(rtn, arr[right]);
				}
				right--;
			}
			left /= 2;
